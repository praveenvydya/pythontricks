...
def getIP():
    ...
    ipaddress = http.request('GET','http://ip.42.pl/raw').read()
    ...
    if response in yes:
        ...
        while 1:
            ...
            if response_b in no:
                ...
                pass
                os.system(
                    'wget http://maps.googleapis.com/maps/api/geocode/json?address=%s -qO .googlemaps.json' % (zipcode))


def getWeather():
    global zipcode, iso_country_code
    os.system(
        'wget "http://api.openweathermap.org/data/2.5/weather?zip=' + zipcode + ',' + iso_country_code + '&units=imperial" -qO .weather.json')
    with open('.weather.json') as data_file:
        weather = json.load(data_file)
    errorcode = weather.get("cod")
    if errorcode == "404":
        print(
        "Sorry! Your area couldn't be found using this API. Try searching for a more general area.")
        exit(1)
